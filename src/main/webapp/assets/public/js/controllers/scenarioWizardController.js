angular.module('smiled.application').controller('scenarioWizardCtrl', ['apiService', '$stateParams', '$state',
  ...
  function scenarioWizardCtrl(apiService, $stateParams, $state, $location, $scope, $element, userService, Upload, CONSTANTS, $q, modalService, $timeout, alertingGeneric) {
        ...
        apiService.getScenario(id).then(
          function (data) {
            ...
            */
            ...
            var real = {
              "name": "Mappa reale",
              "tooltip": "Scegli la mappa selezionando un'area sul planisfero",
              selected: self.scenario.realMap
            };
            var fantasy = {
              "name": "Mappa di fantasia",
              "tooltip": "Carica un'immagine ed utilizzala come mappa per il tuo scenario",
              selected: !self.scenario.realMap
            };
            self.kindOfMap.push(real);
            self.kindOfMap.push(fantasy);
            if (self.scenario.realMap) {
              var m = self.scenario.history.realMap;
              if (m) {
                userService.setTileUrl(m.tileUrl);
                var bounds = {};
                bounds.southWest = {};
                bounds.southWest.lat = m.southWest.x;
                bounds.southWest.lng = m.southWest.y;
                bounds.northEast = {};
                bounds.northEast.lat = m.northEast.x;
                bounds.northEast.lng = m.northEast.y;
                userService.setBounds(bounds);
                var center = {};
                center.lat = m.center.x;
                center.lng = m.center.y;
                center.zoom = m.zoom;
                userService.setCenter(center);
              }
            }
    ...
    self.openRealMapModal = function () {
      ...
      var p = modalService.showRealMapTeacher();
      p.then(
        function (data) {
          saveRealMapInfo();
        },
    ...
    var saveRealMapInfo = function () {
      ...
      if (self.scenario.realMap) {
        ...
        apiService.updateScenario(scenarioDTO, id).then(
            function (data) {
              alertingGeneric.addSuccess("Mappa aggiornata con successo");
            },
            function (reason) {
              alertingGeneric.addWarning("C'è stato un problema con la mappa. Ti preghiamo di riprovare");
            }
          );
      }
    }
