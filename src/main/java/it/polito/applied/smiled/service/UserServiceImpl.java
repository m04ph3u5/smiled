...
//					update.set("profile.coverPhoto", userDTO.getProfile().getCoverPhoto());
	...
	public void registerTeacher(RegisterTeacherDTO teacher) throws UserAlreadyExistsException , MongoException, BadRequestException, MongoDataIntegrityViolationException {
		 ...
		 * cos√¨ da catchare eventuali eccezioni sul DB (MongoException)*/
		try{
			...
			try{
				...
				/*Eccezione che in teoria non dovrebbe mai verificarsi, in quanto abbiamo gi√† controllato che non esista uno user con quella email
				 * al passo precedente*/
				throw e;
			}
		}catch(MongoException e){
		...
		try{
			...
			//if(!teacherAlreadyPresent(emailDTO.getEmail())){
				...
				try{
					...
					System.out.println("Invito non Ë stato fatto poichË l'email era gi‡ presente nel sistema, aggiungo solo il nuovo teacher alla mia lista di colleghi");
